...
finemapping_infiles=${out_dir}/chr${chrom}_finemapping_infile_list.tsv
touch ${finemapping_infiles}
echo "##################Extract sig (nominal or 5e-8) gwas loci for chr${chrom}####################"
## 1.extract all gwas hits
gwas_hit=${out_dir}/gwas_hit.tsv
gwas_hit_sorted=${out_dir}/gwas_hit_sorted.tsv

filtering_snps "${snp_reg}" "${gwas_sig_value}" ${gwas_hit} 0
filtering_snps "${tr_reg}" "${gwas_sig_value}" ${gwas_hit} 1

gwas_hit_n=$(wc -l < ${gwas_hit})
# echo ${gwas_hit_n}
if [[ ${gwas_hit_n} -eq 0 ]]; then
  echo "chr${chrom} has no gwas hit at threshold of ${gwas_sig_value}."
  echo "File prepration for chr${chrom} DONE!"
  exit 0
else
  sort -k2,2n ${gwas_hit} | uniq > ${gwas_hit_sorted} 
#  rm ${gwas_hit}
  ...
  filtering_snps "${snp_reg}" "${nominal_sig_value}" ${nominal_sig} 0
  filtering_snps "${tr_reg}" "${nominal_sig_value}" ${nominal_sig} 1
  # sort variants by p-values
  sort -k2,2n ${nominal_sig} | uniq > ${nominal_sig_sorted}
#  rm ${nominal_sig}
  ...
  while IFS= read -r line; do
    ...
    echo "rsid chromosome position allele1 allele2 maf beta se" > ${curr_folder}/finemap_input.z
