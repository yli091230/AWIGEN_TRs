...
# bcftools +fill-tags ${output_dir}/chr${chrom}_with_af.vcf.gz -Oz -o ${output_dir}/chr${chrom}_with_af_allele_count.vcf.gz -- -t AN,AC
# bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%INFO/AN\n' ${output_dir}/chr${chrom}_with_af_allele_count.vcf.gz > ${output_dir}/chr${chrom}_allele_count.txt
  ${PLINK2} --vcf ${output_dir}/chr${chrom}_with_af.vcf.gz --missing variant-only vcols=+pos --out ${output_dir}/chr${chrom}_F_MISSING
done
